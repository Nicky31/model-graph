...
export default function DataScheme(models, args = {}) {
	...
	// Keys are model names, values are list of models with attribute to autofill with first model 
	this.autolinks = {}
	...
	function EntityModel(name) {
		 ...
		 * 2nd new 'autolinks' parameter allows any linked attribute to be filled 
		 * whenever the linked model is instantiated with a matching attribute
		 */
		this.define = function(links, autolinks={}) {
			...
			this.dependencies =
				dependencies.map(dep => Array.isArray(dep[1]) ? dep[1][0] : dep[1]) // force array conversion if single dependance
			...
			Object.keys(autolinks).forEach(attr => {
				if (!(attr in this.dependencies)) {
					throw ({error: `Model ${this._key} : Trying to set attribute ${attr} which does not correspond to any model dependency as an autolink.`})
				}
	...
	this.allStores = () => Object.keys(this.models).reduce((stores, curName) => {
	...
	function _populate(stores) {
		var clone = Object.assign(Object.create(this.__proto__), this)
	...
	for (let name in models) {
		this.define(name, (typeof models[name] == 'object' ? models[name] : undefined))
	}
}
