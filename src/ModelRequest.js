		...
		return function ModelRequest(input) {
			...
			this.checkNotNull = () => {
				this.result.then(ret => {
					if (!ret) {
						throw ({error: 'Empty request result !'})
					}
				})
				return this
			}
			...
			// If oneshot is false, fn can't be called twice at the same time, but can be performed again as soon as previous call has ended
			...
			// Given some entities and their modelName, look for any linked model in other stores.
			...
			this.fillLinkedModelsWith = (entities, modelName) => {
				...
				autolinks.forEach(autolink => {
					entities.forEach(entity => {
						var id = typeof entity[autolink.via] === 'object' ?
							entity[autolink.via][idAttribute] : entity[autolink.via]
						// Try to find a linked model corresponding to 'id'
						datascheme.request(autolink.linkedModel)
							.get(id)
							.call(linkedModel => {
								if (!linkedModel) {
									return
								...
								if (Array.isArray(updatedAttr) && !(updatedAttr.includes(entity[idAttribute]))) {
									updatedAttr = [...updatedAttr, entity[idAttribute]]
								} else if (!Array.isArray(updatedAttr) && updatedAttr != entity[idAttribute]) {
									updatedAttr = entity[idAttribute]
								}
								
								datascheme.store(autolink.linkedModel)
									.update({
										id,
						...
						.forEach(storeName => {
							...
							if (targetStore == storeName) {
								if (typeof replace !== 'undefined') {
									...
									result = store.update((datas.length < 2 ? datas[0] : datas), { groups })
								}
							} else {
								datascheme.store(storeName).update(datas.length < 2 ? datas[0] : datas)
							}	
						})
