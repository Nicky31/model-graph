...
test("Inserting datas", () => {
	// Filling users & todos
 	return Promise.all([
 		request('users').call(fetchUsers).store().result,
 		request('todos').call(fetchTodos).store().result,
 	])
 	.then(results => {
 		results[0].map(user =>
...
test("Updating datas with group", async () => {
	...
	// Simple update : adding an attribute
 	var result = await request('todos').store({ value: todo, groups: [grpName]}).result
...
test("Deleting datas", async () => {
	...
	await request('todos').value({ id: todoId }).delete().result
...
test("Oneshot saved calls", async () => {
	...
	return request('users').savedCall(savedCall)
		.result.then(result => {
			expect(result).toBe(1)
			return Promise.all([
				new Promise((resolve, reject) => setTimeout(() => resolve(true, 50))),
				request('users').savedCall(savedCall).result.then(result => expect(result).toBe(1))
			])
			.then(() => expect(datascheme.isRunningCall(savedCall.callId)).toBe(false))
		})
})
